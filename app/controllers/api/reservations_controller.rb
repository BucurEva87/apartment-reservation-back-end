module Api
  class ReservationsController < Api::ApplicationController
    def index
      @reservations = current_user.reservations.includes(:apartment, :user)
      render json: { reservations: @reservations.as_json(include: { apartment: {}, user: {} }) }
    end
    ...
    def create
      reservation = Reservation.new(start_date: reservation_params[:start_date],
                                    ...
                                    apartment_id: reservation_params[:apartment_id],
                                    user_id: current_user.id)

      if reservation.save
        render(json: { success: 'Reservation was created', apartment: { id: reservation.id } }, status: 200)
      else
    ...
    def reservation_params
      params.permit(:start_date, :end_date, :apartment_id)
    end
  end
end
