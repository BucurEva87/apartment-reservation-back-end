...
RSpec.describe Reservation, type: :model do
  ...
  let(:apartment) do
    Apartment.create(name: 'Cozy Apartment', description: 'A cozy apartment in the heart of the city',
  ...
  describe 'validations' do
    it 'is valid with valid attributes' do
      reservation = Reservation.new(user:, apartment:, start_date: Date.today,
                                    end_date: Date.today + 1.week)
      expect(reservation).to be_valid
    end

    it 'is not valid without a user' do
      reservation = Reservation.new(apartment:, start_date: Date.today, end_date: Date.today + 1.week)
    ...
    it 'is not valid without a start date' do
      reservation = Reservation.new(user:, apartment:, end_date: Date.today + 1.week)
      expect(reservation).to_not be_valid
    end
    ...
    it 'is not valid if the start date is after the end date' do
      reservation = Reservation.new(user:, apartment:, start_date: Date.today + 1.week,
                                    end_date: Date.today)
      expect(reservation).to_not be_valid
    end

    it 'is not valid if the apartment is already reserved for the same interval' do
      Reservation.create(user:, apartment:, start_date: Date.today,
                         end_date: Date.today + 1.week)
      new_reservation = Reservation.new(user:, apartment:, start_date: Date.today,
                                        end_date: Date.today + 1.week)
